...
def calc_param_SEM_gm(signal_noisy, p, m1, sig1, m2, sig2):
    """
    ...
    proba_apost = p * gausses
    proba_apost = proba_apost / (proba_apost.sum(axis=-1)[..., np.newaxis])
    simu = np.zeros((signal_noisy.shape[0],), dtype=int)
    for i in range(signal_noisy.shape[0]):
        aux = np.random.multinomial(1, proba_apost[i])
        simu[i] = cl[np.argmax(aux)]
    p = calc_probaprio_gm(simu, cl)
    m1 = np.mean(signal_noisy[simu == cl[0]])
    sig1 = np.std(signal_noisy[simu == cl[0]])
    m2 = np.mean(signal_noisy[simu == cl[1]])
    sig2 = np.std(signal_noisy[simu == cl[1]])
    return p, m1, sig1, m2, sig2


def estim_param_SEM_gm(iter, signal_noisy, p, m1, sig1, m2, sig2):
    """
    ...
    :return: Tous les paramètres réestimés à la fin de l'algorithme SEM donc p, m1, sig1, m2, sig2
    """
    p_est = p
    m1_est = m1
    sig1_est = sig1
    ...
    sig2_est = sig2
    for i in range(iter):
        p_est, m1_est, sig1_est, m2_est, sig2_est = calc_param_SEM_gm(signal_noisy, p_est, m1_est, sig1_est, m2_est, sig2_est)
        print(f"[{i+1}/{iter}] p={p_est}, m1={m1_est:.3f}, sig1={sig1_est:.3f}, "f"m2={m2_est:.3f}, sig2={sig2_est:.3f}")
