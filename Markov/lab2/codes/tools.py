...
def gauss(signal_noisy, m1, sig1, m2, sig2):
    ...
    :param m2: La moyenne de la deuxième gaussienne
    :param sig2: La variance de la deuxième gaussienne
    :return: numpy array 2D (longeur de signal_noisy,2) qui correspond aux valeurs des densité gaussiennes pour chaque élément de signal noisy
    """
    gauss1 = (1 / (sig1 * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((signal_noisy - m1) / sig1) ** 2)
    gauss2 = (1 / (sig2 * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((signal_noisy - m2) / sig2) ** 2)
    return np.column_stack((gauss1, gauss2))
